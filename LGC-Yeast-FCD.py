...
from core import leading_tree as lt
from core.delala_select import Fuzzy_select, DeLaLA_select
...
X_length_scale = 1 / (0.02 ** 2)
KernelX = np.exp(-1 * X_length_scale * DistX).T
lt1 = lt.LeadingTree(X_train=KernelX, dc=0.04, lt_num=lt_num, D=D)  # Constructing the lead tree for the entire dataset
lt1.fit()

LTgammaPara = lt1.density * lt1.delta
selectedInds =Fuzzy_select(LTgammaPara, lt1.density, lt1.layer, Y, 2, classNum*2, 0.6)
## select X and Y
...
for runtime in range(Runtimes):
    ...
    givenInds = np.array(selectedInds)
...
for runtime in range(Runtimes):
    ...
    print("Accuracy: ", (count-labeledNum) / (n-labeledNum),", ",count-labeledNum,"/", n-labeledNum)
    ACCArray[runtime] = (count-labeledNum) / (n-labeledNum)

print("Overall Accuracy: ", np.mean(ACCArray),", std ", np.var(ACCArray))
